...
void saveFile(GOODS *goods, int size) {
    FILE* fp;
	...
	for(int i = 0; i < size; i++) {
	    fwrite((goods+i), sizeof(GOODS), 1, fp);
	}
/*	for (int i = 0; i < size; i++) {
		fprintf(fp, "ID: %d, 상품명: %s, 입고량: %d 판매량: %d 가격: %d\n", (goods + i) -> id, (goods + i) -> name, (goods + i) -> receivedCount, (goods + i) -> soldCount, (goods + i) -> price);
	}*/

	fclose(fp);
}

void loadFile(GOODS *goods, int size) {
    FILE* fp;
    int fSize, fCount;
...
int main()
{
    GOODS receivedGoods[5] = {0,};
    int menu;

    while(1) {
        printf("원하는 메뉴를 입력하세요.(1.입고, 2.판매, 3.개별현황, 4.전체현황, 5. 상품 정보 저장, 6.상품 정보 불러오기, 7. 종료)");
        scanf("%d", &menu);
        ...
        switch(menu) {
            ...
            case 4:
                ...
                break;
            case 5:
                saveFile(receivedGoods, 5);
                break;
            case 6:
                loadFile(receivedGoods, 5);
                break;
        }
    }
    return 0;
